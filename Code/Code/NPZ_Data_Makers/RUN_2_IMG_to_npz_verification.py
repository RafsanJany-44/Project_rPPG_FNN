...
def generate_validation_plots_plotly(X_npz, X_live, t, corr, rmse):
    """
    Creates interactive Plotly HTML plots for all 3 ROIs and RGB channels.
    """

    base_name = NPZ_PATH.stem

    # ------------------------------------------------------------
    # 1) Combined overview plot: all 9 channels
    # ------------------------------------------------------------
    ...
    fig_all.update_layout(
        title=f"All ROI RGB Comparison | {base_name}",
        height=1000,
        width=1400,
        template="plotly_white",
    )
    fig_all.update_xaxes(title_text="Time (s)")
    fig_all.update_yaxes(title_text="Intensity")

    out_all = OUT_DIR / f"{base_name}_all_roi_rgb_comparison.html"
    fig_all.write_html(str(out_all), include_plotlyjs="cdn")
    print(f"[Saved] {out_all}")

    # ------------------------------------------------------------
    # 2) Individual ROI plots
    # ------------------------------------------------------------
    for roi_idx, roi_name in enumerate(ROI_DISPLAY_NAMES):
        fig_roi = make_subplots(
            ...
            shared_xaxes=True,
            subplot_titles=[
                f"{roi_name} RGB Stored vs Live",
                f"{roi_name} Residuals (Stored - Live)",
            ],
        )
        ...
        for ch_idx, ch_name in enumerate(CHANNEL_NAMES):
            ...
            residual = X_npz[:, col_idx] - X_live[:, col_idx]
            fig_roi.add_trace(
                go.Scatter(
                    x=t,
                    y=residual,
                    mode="lines",
                    name=f"{ch_name} Residual",
                ),
                row=2,
                col=1,
            )
        ...
        fig_roi.update_layout(
            title=f"{roi_name} RGB Validation | {base_name}",
            height=800,
            width=1400,
            template="plotly_white",
        )
        fig_roi.update_xaxes(title_text="Time (s)", row=2, col=1)
        fig_roi.update_yaxes(title_text="Intensity", row=1, col=1)
        fig_roi.update_yaxes(title_text="Residual", row=2, col=1)

        out_roi = OUT_DIR / f"{base_name}_{roi_name.lower().replace(' ', '_')}_rgb_validation.html"
        fig_roi.write_html(str(out_roi), include_plotlyjs="cdn")
        print(f"[Saved] {out_roi}")

    # ------------------------------------------------------------
    # 3) Original forehead green only plot in Plotly
    # ------------------------------------------------------------
    fg_error = X_npz[:, 1] - X_live[:, 1]

    fig_fg = make_subplots(
        rows=2,
        cols=1,
        shared_xaxes=True,
        subplot_titles=[
            f"Forehead Green Temporal Phase Check | Corr: {corr:.4f}",
            f"Forehead Green Residual | RMSE: {rmse:.4f}",
        ],
    )
    ...
    fig_fg.update_layout(
        title=f"Forehead Green Validation | {base_name}",
        height=800,
        width=1400,
        template="plotly_white",
    )
    fig_fg.update_xaxes(title_text="Time (s)", row=2, col=1)
    fig_fg.update_yaxes(title_text="Intensity", row=1, col=1)
    fig_fg.update_yaxes(title_text="Delta", row=2, col=1)

    out_fg = OUT_DIR / f"{base_name}_forehead_green_validation.html"
    fig_fg.write_html(str(out_fg), include_plotlyjs="cdn")
